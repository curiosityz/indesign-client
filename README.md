...
## New API Endpoint for InDesign Styling

We have introduced a new API endpoint `/applyStyles` that allows you to programmatically apply design styles to a given text directly from InDesign. This endpoint interfaces with Google Gemini for text analysis and applies the resulting styling instructions in InDesign.

### Setting Up

To use this new API endpoint, ensure you have set up your environment with the Google Gemini API key as described in the Google Gemini API Configuration section above.

### Calling the API Endpoint from InDesign

To call the `/applyStyles` endpoint from InDesign, you can use the following example request:

```javascript
const apiUrl = 'http://yourserver.com/applyStyles';
const data = {
  text: 'Your text here',
  documentId: 'InDesignDocumentId'
};
...
This request sends the text and the document ID to the API, which then returns styling instructions that can be applied to the InDesign document.

### Example Response

The API endpoint responds with JSON data containing the styling instructions. Here is an example response:

```json
{
  "message": "Styling instructions generated successfully",
  "stylingInstructions": [
    {
      "index": 0,
      "style": "bold"
...
You can then use these styling instructions to apply styles to your InDesign document programmatically.
